    ...
    async execute({ id, id_academia, nome, descricao, local, sets, tiebreak, modalidade, pontuacao, classesDeleta, classesAdiciona, dataInicio, dataFim }: AtualizarTorneioDTO): Promise<any>{
        ...
        const torneio = await prisma.torneios.update({
            ...
            data: {
                ...
                descricao,
                local,
                sets: torneioExiste.status.id < 4 ? sets : torneioExiste.sets,
                tiebreak: torneioExiste.status.id < 4 ? tiebreak : torneioExiste.tiebreak,
                simples: torneioExiste.status.id < 2 ? modalidade.simples : torneioExiste.simples,
                duplas: torneioExiste.status.id < 2 ? modalidade.duplas : torneioExiste.duplas,
                dataInicio: torneioExiste.status.id < 4 ? dataInicio : torneioExiste.dataInicio,
                dataFim: torneioExiste.status.id < 5 ? dataFim : torneioExiste.dataFim,
            },
            select: {
                id: true,
                nome: true,
